...
/*   Updated: 2024/05/16 17:35:26 by estosche         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd >= MAX_FILE_OPEN || BUFFER_SIZE < 1)
		return (NULL);
	if (fd < 0)
		return (free_gnl(buffers), NULL);
	if (!buffers[fd])
	{
	...
	return (buffers[fd] = copy_buf(buffers[fd], ft_strlen(line)), line);
}
